...
static のメンバークラスは、そのエンクロージングクラスの static メンバーであるので、他の static メンバーと同じアクセス可能性規則に従う。  
...
ローカルクラスは、他の種類のネストしたクラスと共通する特徴を持っている。
...
- コンパイルエラーとなり、コンパイラが Utensil クラスと Dessert クラスの定義が複数回行われていることを知らせてくれる
- pancake と表示される
- potpie と表示される
